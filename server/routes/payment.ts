...
import { getMenuItems, getOrders, addOrder } from '../utils/excelHandler.js';
...
const paymentRateLimit = createRateLimiter(60 * 1000, 40, 'Too many payment attempts. Please wait a moment.');
...
router.post('/verify', paymentRateLimit, async (req, res) => {
  try {
    ...
    const duplicateOrder = existingOrders.find(o => o.payment_id === razorpay_payment_id);
    if (duplicateOrder) {
    ...
    const result = await withWriteLock(async (lockedWb) => {
      const currentMenu = getMenuItems(lockedWb); 
      ...
      addOrder(newOrderRecord, lockedWb);
    ...
    const activeOrders = existingOrders.filter(o => o.order_status === 'PLACED' || o.order_status === 'PREPARING');
